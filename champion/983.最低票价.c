...
int mincostTickets( int* days, int daysSize, int* costs, int costsSize ) {
    ...
    for ( i = 1; i <= max_date; i++ ) {
        ...
        temp1   = dp[ i - 1 ] + costs[ 0 ];
        temp2   = dp[ i - 7 > 0 ? i - 7 : 0 ] + costs[ 1 ];
        temp3   = dp[ i - 30 > 0 ? i - 30 : 0 ] + costs[ 2 ];
        dp[ i ] = temp1 > temp2 ? temp2 : temp1;
        dp[ i ] = dp[ i ] > temp3 ? temp3 : dp[ i ];
    }
    return dp[ max_date ];
}
